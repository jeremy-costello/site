...
docker rm -fv llamacpp-wasm-builder 2>/dev/null || true # Ensure container is removed
docker rmi $(docker inspect --format='{{.Image}}' llamacpp-wasm-builder) 2>/dev/null || true # Remove its image
